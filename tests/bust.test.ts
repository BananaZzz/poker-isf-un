...
describe('bust behavior at engine level', () => {
  ...
  it('funded players continue when one busts (excluded from next hand)', () => {
    ...
    // now runout to showdown (both effectively all-in / no more chips for a)
    // engine advances automatically through streets when no active players remain
    expect([GamePhase.HAND_COMPLETE, GamePhase.SHOWDOWN]).toContain(s.phase);
